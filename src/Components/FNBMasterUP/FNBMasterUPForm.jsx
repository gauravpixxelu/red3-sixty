    ...
    const FNBMasterUPForm = () => {
        ...
        const [isBusinessAgeSelected, setIsBusinessAgeSelected] = useState(false);
        const [isEmployesSelected, setIsEmployesSelected] = useState(false);
        const [isAnnualTurnoverSelected, setAnnualTurnoverSelected] = useState(false);
        const [isFormSubmitted, setIsFormSubmitted] = useState(false); // Add a new state variable


        const handleSubmit = async (e) => {
            e.preventDefault();
            if (!isBusinessAgeSelected) {
                alert('Please select a Business Age.');
                return;
            }
            if (!isEmployesSelected) {
                alert('Please select Number of Employes.');
                return;
            }
            if (!isAnnualTurnoverSelected) {
        ...
        return (
            ...
            <form onSubmit={handleSubmit} className={`from-bottom ${thankmessage ? 'success' : ''}`}>
                <div className="row">
                    ...
                    <div className="col-md-6">
                        <div className="form-feild" required>
                            <label className="fnb-label">Business Age (years)</label>
                            <div className="radio-gd">
                                <div><label><input type="radio" value="0-5" checked={business_age === '0-5'} onChange={(e) => { setBusinessAge(e.target.value); setIsBusinessAgeSelected(true); }} required />0-5</label></div>
                                <div><label><input type="radio" value="5-10" checked={business_age === '5-10'} onChange={(e) => { setBusinessAge(e.target.value); setIsBusinessAgeSelected(true); }} required />5-10</label></div>
                                <div><label><input type="radio" value="10-15" checked={business_age === '10-15'} onChange={(e) => { setBusinessAge(e.target.value); setIsBusinessAgeSelected(true); }} required />10-15</label></div>
                                <div><label><input type="radio" value="15-20" checked={business_age === '15-20'} onChange={(e) => { setBusinessAge(e.target.value); setIsBusinessAgeSelected(true); }} required />15-20</label></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="col-md-6">
                        <div className="form-feild" required>
                            ...
                            <div className="radio-gd">
                                <div><label><input type="radio" value="0-10" checked={number_of_employee === '0-10'} onChange={(e) => { setEmployes(e.target.value); setIsEmployesSelected(true); }} required />0-10</label></div>
                                <div><label><input type="radio" value="10-20" checked={number_of_employee === '10-20'} onChange={(e) => { setEmployes(e.target.value); setIsEmployesSelected(true); }} required />10-20</label></div>
                                <div><label><input type="radio" value="20-30" checked={number_of_employee === '20-30'} onChange={(e) => { setEmployes(e.target.value); setIsEmployesSelected(true); }} required />20-30</label></div>
                                <div><label><input type="radio" value="30-50" checked={number_of_employee === '30-50'} onChange={(e) => { setEmployes(e.target.value); setIsEmployesSelected(true); }} required />30-50</label></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="col-md-12">
                        <div className="form-button fnb-btn">
                            <div className="fnb-submit">
                                <button type="submit" className="custom-btn" disabled={isFormSubmitted}><span>{isFormSubmitted ? 'Submitting...' : 'Proceed'}</span></button>
